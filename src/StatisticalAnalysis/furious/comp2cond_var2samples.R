...
comp2cond.var2samples = function(cds, i, j, cond1, cond2, allres, nb, genes.res, tlfc, tpadj) {
	...
	allres[   which (rownames(allres) %in% res$id[res$padj<tpadj & res$log2FoldChange>1]) , nb ] = 1
	allres[   which (rownames(allres) %in% res$id[res$padj<tpadj & res$log2FoldChange<(-1)]) , nb ] = -1
	colnames(allres)[nb] = paste(cond2,"vs",cond1)
	
	#scatter plot
	pdf(paste("figures/",cond2,"_vs_",cond1,"_scatterplot.pdf", sep=""))
	suppressWarnings(plot(res$baseMeanA, res$baseMeanB, log="xy", pch=10, cex=0.6, col=ifelse(res$padj<tpadj & abs(res$log2FoldChange) > tlfc, "red", "black") , cex.lab=1, cex.axis=1, 
			xlab= cond1, ylab=cond2, xlim = c(MinValue,MaxValue), ylim = c(MinValue,MaxValue)))
	legend("bottomright", col=c(2,1), legend=c("FDR<threshold and |lfc|>1","FDR>threshold or |lfc|<1"), pch=20, cex=0.9)
	dev.off()
